...
const LegalDraftTemplates = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const basePromptValue = `You are an expert Indian legal document drafter. Create professional legal documents following Indian court standards and formats. Please consider the following details:
...
Ensure the draft is court-ready, professionally structured, and follows all Indian legal drafting standards. The document should be suitable for filing in the specified jurisdiction.`;
    ...
    const documentContext = documentText ? `\n\nAdditional Document Context:\n${documentText}\n\nPlease incorporate this document content into the legal draft as appropriate.` : '';

    const finalPrompt = `${basePromptValue}${documentContext}`;

      const result = await callGeminiAPI(finalPrompt, geminiKey);
      // Clean result: remove asterisks for cleaner formatting
      const cleanedResult = result.replace(/\*/g, "");

      setGeneratedTemplate(cleanedResult);
      setShowEditor(true);
      toast({
                ...
                <div>
                  ...
                  {documentText && (
                    <div className="mt-2 p-3 bg-gray-50 rounded-md">
                      <p className="text-sm text-gray-600">Document uploaded successfully. Content will be used in template generation.</p>
                    </div>
                  )}
                </div>
